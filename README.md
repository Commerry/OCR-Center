# OCR Center

เว็บกลางสำหรับมอนิเตอร์กล้อง OCR (CM4) ทั้ง fleet — รับ heartbeat จากกล้อง, ลงทะเบียนอุปกรณ์อัตโนมัติ, เก็บประวัติค่าที่อ่านได้, แสดงสถานะ/สุขภาพเครื่องแบบเรียลไทม์

## โครงสร้าง

```
OCR-Center/
├── package.json
├── .env                  # ตั้งค่า PORT / API_KEY / retention
├── data/center.db        # SQLite (สร้างอัตโนมัติ)
└── src/
    ├── server.js         # express bootstrap + basic-auth dashboard
    ├── db.js             # schema + ingest transaction + prune
    ├── routes/
    │   ├── ingest.js     # POST /api/devices/heartbeat  (ฝั่งกล้อง, X-Api-Key)
    │   └── api.js        # REST ฝั่ง dashboard
    └── public/index.html # หน้า fleet dashboard (ธีมเดียวกับหน้ากล้อง)
```

## ติดตั้ง + รัน

**ทางลัด (Linux/Raspberry Pi — ติดตั้ง + pm2 + auto-start ตอนบูต ครบในคำสั่งเดียว):**

```bash
git clone https://github.com/Commerry/OCR-Center.git
cd OCR-Center
bash install.sh
```

**หรือรันมือ (ทุก OS รวม Windows):**

```bash
npm install
cp .env.example .env    # Windows: copy .env.example .env
npm start               # http://localhost:8090
```

> เครื่องหลัง proxy โรงงาน: ตั้ง proxy ของ npm ก่อน (`npm config set proxy http://10.201.0.54:8080` + `https-proxy` + `strict-ssl false`) — รายละเอียดดู `docs/UPDATE_GUIDE.md` ใน repo กล้อง

## เชื่อมกล้องเข้า Center

ที่หน้าเว็บกล้องแต่ละตัว → **System → Central Server API**:

- Central API URL: `http://<IP เครื่องที่รัน Center>:8090/api/devices/heartbeat`
- API Key: ให้ตรงกับ `API_KEY` ใน `.env` (ถ้าตั้ง)
- เปิด toggle "ส่งข้อมูลไปเว็บกลาง" → Save API Settings

อุปกรณ์จะโผล่บน dashboard เองเมื่อ heartbeat แรกเข้ามา (จำด้วย `deviceId` = MAC address) — ไม่ต้องแอดมือ

## ข้อมูลที่เก็บ

| ตาราง | ข้อมูล | retention |
|---|---|---|
| `devices` | ตัวตนอุปกรณ์ + health ล่าสุด | ถาวร |
| `cameras` | สถานะกล้อง/PLC/ค่าอ่าน/ภาพล่าสุด ต่อกล้อง | ล่าสุดเสมอ |
| `reads` | ประวัติเลขทุกค่าที่ส่ง PLC + น้ำหนัก + โมเดลที่ใช้ตอนนั้น (กันซ้ำด้วย device+camera+เวลา) | 90 วัน |
| `health_history` | temp/CPU/RAM/disk sample ทุก 5 นาที | 14 วัน |

## Alert rules (คำนวณอัตโนมัติ)

- **OFFLINE** — ไม่มี heartbeat เกิน `OFFLINE_AFTER_SEC` (default 90 วิ)
- **Disk** ≥75% เหลือง, ≥85% แดง
- **Temp** ≥65°C เหลือง, ≥75°C แดง
- **camera enabled แต่ไม่ running** / **PLC enabled แต่ต่อไม่ได้**

## การจัดกลุ่ม (Group / Plant)

อุปกรณ์ใหม่เข้ามาอยู่ "ยังไม่จัดกลุ่ม" เสมอ → จัดกลุ่มจากหน้าเว็บ (ไม่ต้องแตะกล้อง):

- **New Group** บน topbar → สร้างกลุ่ม (เช่น โรงงาน A, ไลน์ผลิต 1)
- ปุ่มลูกศรบนการ์ดอุปกรณ์ → ย้ายเข้ากลุ่ม / สร้างกลุ่มใหม่พร้อมย้าย
- หัวข้อกลุ่ม: คลิกพับ/กาง (จำสถานะไว้), ปุ่มแก้ชื่อ/ลบกลุ่ม (ลบแล้วสมาชิกกลับไป Unassigned)
- ทุกกลุ่มอยู่หน้าเดียว แสดง online/total + alerts ราย.กลุ่ม

API: `GET/POST /api/groups`, `PATCH/DELETE /api/groups/:id`, `POST /api/devices/:id/group {groupId}`

## รายงาน (Report)

กดปุ่ม **Report** บน topbar เพื่อออกรายงานการอ่านตัวเลขเป็นไฟล์ ZIP

**เลือกได้:**

- ช่วงเวลา (มีปุ่มลัด วันนี้ / เมื่อวาน / 7 วัน / 30 วัน)
- ขอบเขต: ทุกอุปกรณ์ / เลือกทั้งกลุ่ม / ติ๊กเลือกอุปกรณ์เอง
- เกณฑ์ความมั่นใจขั้นต่ำ (%) สำหรับทำเครื่องหมายรายการที่ควรตรวจซ้ำ
- แนบรูปที่อุปกรณ์ส่งมาหรือไม่

ก่อนดาวน์โหลดจะเห็นตัวเลขสรุปก่อน (อ่านทั้งหมด / สำเร็จ / ผิด / มีรูป / อัตราความถูกต้อง)

**ไฟล์ ZIP ที่ได้:**

| ไฟล์ | เนื้อหา |
|---|---|
| `detail.csv` | รายการอ่านทีละครั้งตามเวลา: วันที่เวลา, กลุ่ม, อุปกรณ์, IP, กล้อง, เลขที่อ่านได้, สถานะ, ความมั่นใจ %, ต่ำกว่าเกณฑ์, โมเดลที่ใช้, อ่านตัวอักษรนำหน้า, น้ำหนัก, ชื่อไฟล์รูป |
| `summary.csv` | สามส่วน: สรุปรายอุปกรณ์ (โมเดลที่ใช้/อ่านทั้งหมด/สำเร็จ/888/999/อัตราความถูกต้อง/ความมั่นใจเฉลี่ย/น้ำหนักเฉลี่ย-ต่ำสุด-สูงสุด), สรุปรายโมเดล (เทียบความแม่นระหว่างโมเดลบนข้อมูลชุดเดียวกัน) และสรุปรายเลขที่อ่านได้ (จำนวนครั้ง, สัดส่วน, ความมั่นใจเฉลี่ย/ต่ำสุด, น้ำหนักเฉลี่ย) |
| `images/...` | รูปที่อุปกรณ์ส่งมาของแต่ละครั้งที่อ่าน อ้างอิงจากคอลัมน์ "ไฟล์รูป" ใน detail.csv |

ไฟล์ถูกสร้างใน worker thread แยก และอ่านข้อมูลทีละแถว หน้าเว็บกับ heartbeat จึงทำงานต่อได้ระหว่างสร้างรายงาน (ทดสอบที่ 300,000 แถว + รูป 2,000 ใบ: ZIP 50 MB ใน 11 วินาที แรมสูงสุด 160 MB) ไฟล์ชั่วคราวเก็บที่ `data/tmp` เปลี่ยนได้ด้วย `REPORT_TMP_DIR`

CSV ใส่ BOM มาให้แล้ว เปิดด้วย Excel ภาษาไทยไม่เพี้ยน เวลาทุกคอลัมน์เป็นเวลาไทยตามค่า `REPORT_TZ`

**โมเดลกับน้ำหนัก:** โมเดลมาจากค่าที่ตั้งไว้ในหน้ากล้อง (ช่องว่าง = โมเดลเริ่มต้นที่ติดมากับโปรแกรม) บันทึกไว้กับทุกแถวที่อ่าน จึงย้อนดูได้ว่าเลขนั้นอ่านด้วยโมเดลตัวไหน ส่วนน้ำหนักคือค่าที่ PLC ส่งมาใกล้เคียงเวลาที่อ่านมากที่สุด (กล้องอ่านค่าน้ำหนักทุก 0.5 วิ) ถ้าไซต์ไหนไม่ได้ต่อค่าน้ำหนักจาก PLC ช่องนี้จะว่าง ข้อมูลสองอย่างนี้เริ่มเก็บจากรอบที่อัปเดตโปรแกรมทั้งฝั่งกล้องและ Center — แถวเก่าก่อนหน้านั้นจะว่าง (สรุปรายโมเดลจะขึ้นเป็น "ค่าเริ่มต้น (default)")

**นิยามที่ใช้คำนวณ:** ค่า 888 คืออ่านไม่เจอตัวเลข, 999 คือรูปแบบตัวเลขไม่ถูกต้อง ทั้งสองนับเป็น "อ่านผิด" ที่เหลือนับเป็น "อ่านสำเร็จ" อัตราความถูกต้อง = อ่านสำเร็จ ÷ อ่านทั้งหมด

## การเก็บรูปที่อุปกรณ์ส่งมา

Center บันทึกรูปทุกครั้งที่อุปกรณ์ส่ง heartbeat พร้อมรูปใหม่ลงที่ `data/images/<deviceId>/<วันที่>/` (รูปเดิมที่ส่งซ้ำจะไม่บันทึกซ้ำ)

### กันพื้นที่เต็ม — 3 ชั้น

| ชั้น | ตัวแปรใน `.env` | ทำอะไร |
|---|---|---|
| อายุ | `IMAGES_KEEP_DAYS=30` | ลบรูปที่เก่ากว่ากี่วัน (0 = ไม่ลบตามอายุ) ตรวจทุกชั่วโมง |
| **เพดานพื้นที่** | `IMAGES_MAX_MB=20000` | เพดานแข็ง เกินเมื่อไหร่ลบโฟลเดอร์ "วันเก่าสุด" ทิ้งจนต่ำกว่าเพดาน (0 = ไม่จำกัด) — ตัวนี้คือหลักประกันว่ารูปจะไม่มีวันทำให้ดิสก์เต็ม ไม่ว่าตั้งจำนวนวันไว้เท่าไหร่ |
| เบรกฉุกเฉิน | `IMAGES_MIN_FREE_PERCENT=15` | ดิสก์เหลือน้อยกว่านี้ = หยุดเขียนรูปใหม่ทันที (heartbeat/เลข/สถานะยังทำงานปกติ) กันกรณีโปรแกรมอื่นบนเครื่องกินพื้นที่ |

เพดานถูกเช็ค 2 จังหวะ: ทุก heartbeat (เช็คจากขนาดที่แคชไว้ในหน่วยความจำ ไม่สแกนดิสก์) และตอน prune รายชั่วโมง เวลาโดนลบ ทั้งไฟล์และแถวใน `read_images` หายไปพร้อมกัน รายงานเก่าจึงไม่มีช่องรูปที่ชี้ไปไฟล์ที่ไม่มีอยู่

### เลือกว่ารูปไหนคุ้มที่จะเก็บ

`IMAGE_STORE_MODE`:

| ค่า | เก็บอะไร | พื้นที่โดยประมาณ |
|---|---|---|
| `all` (ค่าเริ่มต้น) | ทุกครั้งที่อ่าน | 50 กล้อง × 300 ครั้ง/วัน ≈ 750 MB/วัน (~22 GB/เดือน) |
| `smart` | อ่านผิด (888/999) + ความมั่นใจต่ำกว่า `IMAGE_LOW_CONF_PERCENT=90` + สุ่ม `IMAGE_SAMPLE_PERCENT=5` % ของใบที่อ่านถูก | ~15% ของ `all` (~3 GB/เดือน) |
| `failed` | เฉพาะ 888/999 | น้อยสุด แต่ไม่มีรูปที่อ่านถูกไว้เทียบตอน retrain |

ย้ายที่เก็บออกจากไดรฟ์ระบบได้ด้วย `IMAGES_DIR=/mnt/data/ocr-images` (Windows: `D:\ocr-images`) เว้นว่าง = `data/images` ข้างโปรแกรม

### ดูสถานะ

บนหัวเว็บมีชิปบอกพื้นที่ที่ใช้ / เพดาน (เหลืองที่ 75%, แดงที่ 90% หรือเมื่อหยุดเก็บ) เอาเมาส์ชี้เพื่อดูรายละเอียด: จำนวนรูป, ดิสก์เหลือ %, โหมดที่ใช้, ช่วงวันที่มีรูป, ที่เก็บไฟล์

หรือเรียกเอง: `GET /api/reports/storage`

> อุปกรณ์ส่งรูปมาให้รอบละ 1 ใบ (รูปของการอ่านครั้งล่าสุด) ถ้ามีการอ่านหลายครั้งภายในรอบเดียวกัน จะมีรูปเฉพาะครั้งท้ายสุด แถวอื่นใน detail.csv จะเว้นช่องไฟล์รูปไว้

## ออกรายงานจากบรรทัดคำสั่ง (ไม่ต้องผ่านเว็บ)

ใช้ได้เสมอแม้เบราว์เซอร์มีปัญหา และใช้ดูว่าช้าตรงไหน (บอกจำนวนแถว เวลาที่ใช้ และแรมสูงสุด):

```bash
cd ~/Desktop/OCR-Center-main
node tools/export-report.js --count-only --days 30        # ดูว่ามีกี่แถว กี่รูป ก่อนออกจริง
node tools/export-report.js --days 7 --out ~/report.zip   # ออกจริง 7 วันล่าสุด
node tools/export-report.js --days 30 --no-images         # ไม่แนบรูป ไฟล์เล็กและเร็วกว่ามาก
node tools/export-report.js --from 2026-09-01 --to 2026-09-23
node tools/export-report.js --device d8:3a:dd:42:a4:ef --days 1
```

## นาฬิกากล้อง - Center เป็นตัวอ้างอิงให้เอง

CM4 ไม่มีถ่าน RTC ไฟดับแล้วเวลาย้อนกลับไปหลายสัปดาห์ ทุกค่าที่อ่านได้จะถูกประทับเวลาผิด Center เก็บไว้ในวันเก่า รายงานของสัปดาห์ปัจจุบันจึงว่างทั้งที่กล้องทำงานปกติ

Center ตอบเวลาของตัวเองกลับไปกับ heartbeat ทุกครั้ง:

```json
{ "ok": true, "serverTime": "2026-09-26T03:00:00.000Z", "timeZone": "Asia/Bangkok", "driftSec": 4752000 }
```

กล้องเทียบกับนาฬิกาตัวเอง ถ้าต่างเกิน 5 วินาทีจะตั้งใหม่ทันที (ตั้งเป็น UTC จึงไม่มีปัญหาเรื่อง timezone) พร้อมเขียนลง hwclock/fake-hwclock ให้รอดการรีบูต — heartbeat แรกหลังไฟดับก็แก้ให้เองแล้ว ไม่ต้องมี NTP ซึ่งกล้องออกไปไม่ได้อยู่แล้วเพราะติด proxy

`timeZone` มาจาก `REPORT_TZ` ใน `.env` ของ Center ตั้งที่เดียวใช้ทั้ง fleet
เวลาต่างเกิน 2 นาทีจะมีบรรทัดใน log ของ Center บอกว่าอุปกรณ์ไหนถูกแก้

> ฝั่งกล้องต้องมีสิทธิ์ตั้งเวลา ติดตั้งโดย `install.sh`/`update.sh` อัตโนมัติ (เฉพาะคำสั่งเกี่ยวกับเวลาเท่านั้น) กล้องที่อัปเดตมาจากเวอร์ชันเก่าต้องรัน `sudo bash tools/install-timesync-sudoers.sh` ครั้งเดียว

## รายงานออกมาเปล่า - ตรวจว่ามีข้อมูลจริงไหม

```bash
cd ~/Desktop/OCR-Center-main
node tools/check-data.js --days 7
```

บอกทีละอุปกรณ์ว่ามี reads กี่แถว อยู่ในช่วงที่เลือกกี่แถว และอ่านล่าสุดเมื่อไหร่
อุปกรณ์ที่ขึ้น 0 ในช่วงที่เลือก = เลือกตัวนั้นแล้วจะได้รายงานเปล่า สาเหตุที่พบบ่อยคือกล้องยังไม่ได้เปิดส่งข้อมูลไป Center, เพิ่งต่อเข้าระบบ, หรือนาฬิกากล้องเพี้ยน

ตั้งแต่เวอร์ชัน 1.2.0 ถ้าขอบเขตไม่มีข้อมูล ระบบจะไม่ส่งไฟล์เปล่าให้ แต่จะบอกเหตุผลบนหน้าเว็บแทน

## เช็คว่าเครื่องรันเวอร์ชันไหนอยู่

```bash
curl -s http://localhost:8090/api/health
```
ตอบ `{"ok":true,"version":"1.2.0","node":"...","uptimeSec":...}` — ไม่ต้อง login เอาไว้ยืนยันว่าอัปเดตขึ้นจริงแล้ว

## Dashboard API

- `GET /api/summary` — จำนวน online/offline/alerts
- `GET /api/devices` — รายการอุปกรณ์ + กล้อง + alerts
- `GET /api/devices/:id` — รายละเอียดตัวเดียว
- `GET /api/devices/:id/reads?limit=100` — ประวัติการอ่าน
- `GET /api/devices/:id/health-history?limit=288` — ประวัติสุขภาพเครื่อง
- `GET /api/devices/:id/cameras/:camera/image` — ภาพล่าสุด (webp)
- `POST /api/reports/preview` — นับจำนวนก่อนออกรายงาน (from, to, groupId หรือ deviceIds, minConfidence)
- `POST /api/reports/export` — สร้างรายงาน ZIP (พารามิเตอร์เดียวกับ preview + includeImages)
- `GET /api/reports/export?from=&to=&deviceIds=&groupId=&minConfidence=&includeImages=` — ลิงก์ดาวน์โหลดที่หน้าเว็บใช้ (เบราว์เซอร์เขียนไฟล์ลงดิสก์โดยตรง ไม่ผ่านหน่วยความจำของแท็บ)
- `GET /api/reports/storage` — จำนวนรูปและพื้นที่ที่ใช้เก็บ

สเปก payload ฝั่งกล้องอยู่ที่ `OCR-V8.1/src/utils/centralReporter.js`

## ตั้งชื่อเว็บ + Login

ตั้งใน `.env`:

```
SITE_NAME=PSE OCR CENTER
SITE_SUBTITLE=Vision Fleet Monitor
DASH_USER=admin
DASH_PASS=your-password
```

- `SITE_NAME` / `SITE_SUBTITLE` — ชื่อที่แสดงบนหัวเว็บ, หน้า login และแท็บเบราว์เซอร์
- `DASH_USER` / `DASH_PASS` — บัญชีเข้าใช้งาน (เว้น `DASH_USER` ว่าง = ไม่ต้อง login)
- แก้ `.env` แล้วต้อง `pm2 restart ocr-center`

## URL หน้าเว็บของอุปกรณ์

ปุ่มลิงก์บนการ์ดใช้ `http://<IP ที่กล้องส่งมา>:64010` อัตโนมัติ — แก้เป็น URL อื่นได้รายอุปกรณ์:
คลิกการ์ด → ปุ่มรูปโซ่ (ข้างปุ่มลบ) → ใส่ URL เต็ม (เว้นว่าง = กลับไปใช้ค่าอัตโนมัติ)

## ความปลอดภัย

- Dashboard: ตั้ง `DASH_USER`/`DASH_PASS` ใน `.env` เพื่อเปิด basic-auth (ค่าว่าง = ไม่ล็อค)
- ฝั่งกล้อง: ตั้ง `API_KEY` แล้วใส่ key เดียวกันในหน้ากล้อง
